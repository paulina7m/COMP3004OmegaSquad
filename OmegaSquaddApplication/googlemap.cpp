#include "googlemap.h"
    ...
    void GoogleMap::updateIcons(int index){
        QString indexString = "";
        indexString.setNum(index+1);
        QHash<QString, QString>::iterator i = unitLocations->begin();
                 while (i != unitLocations->end()) {
                     qDebug() << "key is " << i.key();
                     QString iconurl = this->iconURLs->value(indexString + "-1");
                     //TODO:
                     //correct the icon colors here
                      page()->mainFrame()->evaluateJavaScript("marker"+ i.key() + ".setIcon(\"" + iconurl + "\");");
                      ++i;
                 }
    }
